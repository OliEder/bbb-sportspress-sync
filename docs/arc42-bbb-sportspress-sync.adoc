...
Zusätzlich: Meta-Boxen auf `sp_player` (Person-ID) und `sp_team` (Team Permanent-ID) für manuelle Datenpflege.
...
=== Meta-Boxen für manuelle Datenpflege

Admin-UI registriert Meta-Boxen auf:

* `sp_player` Edit-Screen: Feld für `_bbb_person_id` (BBB Person-ID)
* `sp_team` Edit-Screen: Feld für `_bbb_team_permanent_id` (BBB Team Permanent-ID)
