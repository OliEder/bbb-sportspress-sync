...
class BBB_Admin_Page {
    ...
    private function determine_league_type( string $table_exists_meta ): string {
        return '0' === $table_exists_meta ? 'tournament' : 'league';
    }
        ...
        if ( $deleted > 0 ) {
            if ( 'all' !== $team_filter ) {
                ...
                if ( $event_ids ) {
                    ...
                    $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->postmeta} WHERE meta_key = '_bbb_boxscore_synced' AND post_id IN ($placeholders)", ...$event_ids ) ); // nosemgrep: php.lang.security.injection.tainted-callable.tainted-callable,php.lang.security.injection.tainted-sql-string.tainted-sql-string -- false positive: safe variadic wpdb-prepare IN() pattern.
                }
            } else {
